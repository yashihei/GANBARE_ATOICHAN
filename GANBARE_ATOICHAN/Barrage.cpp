...
void Barrage1::move(Vec2 ownerPos, Vec2 targetPos, int cnt, BulletManager* bulletManager)
{
	...
	if (cnt % interval == 0) bulletManager->create(ownerPos, { Sin(rad)*sp, Cos(rad)*sp }, Color(255, 0, 255, 200), 5.0);
}
...
void Barrage2::move(Vec2 ownerPos, Vec2 targetPos, int cnt, BulletManager* bulletManager)
{
	...
	if (cnt % interval == 0) {
		for (int i = 0; i < sep; i++) {
			bulletManager->create(ownerPos, { Sin(rad + PI2 / sep * i)*sp, Cos(rad + PI2 / sep * i)*sp }, Color(0, 255, 255, 200), 5.0);
		}
	}
}
